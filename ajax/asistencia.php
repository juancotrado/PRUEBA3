...
switch ($_GET["op"]) {
	...
	date_default_timezone_set('America/Lima');
          ...
          if ($par == 0){ 
    			...
    			//$movimiento = 0;
    			echo $rspta ? '<h3>Nombres: '. $result['nombre'].' '.$result['apellidos'].'</h3><div class="alert alert-success"> Ingreso registrado '.$hora.'</div>' : 'No se pudo registrar el ingreso';
   		  }else{ 
                $tipo = "Salida";
         		$rspta=$asistencia->registrar_salida($codigo_persona,$tipo,$detalles,$evidencia);
     			//$movimiento = 1;
     			echo $rspta ? '<h3>Nombres: '. $result['nombre'].' '.$result['apellidos'].'</h3><div class="alert alert-danger"> Salida registrada '.$hora.'</div>' : 'No se pudo registrar la salida';             
        } 
        } else {
