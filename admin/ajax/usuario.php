...
$tipdepersonal=isset($_POST["tipdepersonal"])? limpiarCadena($_POST["tipdepersonal"]):"";
$grupocupacional=isset($_POST["grupocupacional"])? limpiarCadena($_POST["grupocupacional"]):"";
...
$idcat=isset($_POST["idcat"])? limpiarCadena($_POST["idcat"]):"";
...
switch ($_GET["op"]) {
	case 'guardaryeditar':
		...
		if (empty($idusuario)) {
			$for = $_POST['idusuario'];
			if($iddepartamento=="1"){
				$rspta=$usuario->insertar($nombre,$apellidos,$login,$tipdepersonal,$iddepartamento,$grupocupacional,$idcat);
				echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar todos los datos del usuario";
			}
			else{
				$rspta=$usuario->insertar1($nombre,$apellidos,$login,$tipdepersonal,$iddepartamento,$grupocupacional,$idcat);
				echo $rspta ? "Datos registrados correctamente" : "No se pudo registrar todos los datos del usuario";
		
			}
			}
		else {
			$rspta=$usuario->editar($idusuario,$nombre,$apellidos,$login,$iddepartamento,$idtipousuario,$idcargo,$tipdepersonal,$grupocupacional,$email,$codigo_persona);
	...
	case 'selectCat':
			$for = $_POST['idusuario'];
			$rspta=$usuario->select1($for);
			
			echo '<option value="0">seleccione...</option>';
			while ($reg=$rspta->fetch_object()) {
				echo '<option value=' . $reg->idcat.'>'.$reg->nombre.'</option>';
			}
	break;
